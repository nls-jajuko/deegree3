...
/**
 * Stream-based GML writer for {@link Feature} (and {@link FeatureCollection})
 * instances.
 * 
...
public class GMLFeatureWriter extends AbstractGMLObjectWriter {
    ...
    private static final Logger LOG = LoggerFactory
            .getLogger(GMLFeatureWriter.class);
    ...
    private final Stack<QName> exportContext = new Stack<QName>();    
    private final ArrayList<ValueReference> requestedPropertyPaths = new ArrayList<ValueReference>();
    ...
    public GMLFeatureWriter(GMLStreamWriter gmlStreamWriter) {
            ...
            for (ProjectionClause projection : gmlStreamWriter.getProjections()) {
                if (projection instanceof PropertyName) {
                    ...
                    if (qName != null) {
                        ...
                        requestedPropertyPaths.add(propName.getPropertyName());
                    ...
                    timeSliceFilters.add(((TimeSliceProjection) projection)
                            .getTimeSliceFilter());
                }
            }
        }
        ...
        this.boundedByPt = new EnvelopePropertyType(new QName(gmlNs,
                "boundedBy"), 0, 1, null, null);
        this.exportBoundedBy = gmlStreamWriter
                .getGenerateBoundedByForFeatures();
    }
     ...
     * @throws TransformationException
     */
    public void export(Property prop) throws XMLStreamException,
            UnknownCRSException, TransformationException {
    ...
    public void export(TypedObjectNode node, GmlXlinkOptions resolveState)
        ...
        if (node instanceof GMLObject) {
            ...
            for (TypedObjectNode elem : ((TypedObjectNodeArray<?>) node)
                    .getElements()) {
    ...
    private void export(Property property, GmlXlinkOptions resolveState)
        ...
        exportContext.push(property.getName());
        try {

            QName propName = property.getName();
            PropertyType pt = property.getType();
            if (pt.getMinOccurs() == 0) {
                LOG.debug("Optional property '" + propName
                        + "', checking if it is requested.");
            ...
            // TODO check for GML 2 properties (gml:pointProperty, ...) and
            // export
            ...
            if (pt instanceof FeaturePropertyType) {
                if (nilled) {
                    writeEmptyElementWithNS(propName.getNamespaceURI(),
                            propName.getLocalPart());
                    writeAttributeWithNS(XSINS, "nil", "true");
                    endEmptyElement();
                } else {
                    exportFeatureProperty((FeaturePropertyType) pt,
                            (Feature) value, resolveState);
                }
                ...
                if (nilled) {
                    ...
                    if (!exportSf
                            && gValue.getId() != null
                            && referenceExportStrategy.isObjectExported(gValue
                                    .getId())) {
                        writeEmptyElementWithNS(propName.getNamespaceURI(),
                                propName.getLocalPart());
                        writeAttributeWithNS(XLNNS, "href",
                                "#" + gValue.getId());
                        endEmptyElement();
                    } else {
                        writeStartElementWithNS(propName.getNamespaceURI(),
                        ...
                        if (gValue.getId() != null) {
                            ...
                            writer.writeComment("Inlined geometry '"
                                    + gValue.getId() + "'");
                        }
                        gmlStreamWriter.getGeometryWriter().export(
                                (Geometry) value);
                        writer.writeEndElement();
                    }
                }
                ...
                if (codeType != null) {
                    if (codeType.getCodeSpace() != null
                            && codeType.getCodeSpace().length() > 0) {
                        if (GML_2 != version) {
                            writer.writeAttribute("codeSpace",
                ...
                if (nilled) {
                    ...
                    writeStartElementWithNS(propName.getNamespaceURI(),
                            propName.getLocalPart());
                    if (value != null) {
                        gmlStreamWriter.getGeometryWriter().exportEnvelope(
                                (Envelope) value);
                    } else {
                ...
                if (stringOrRef.getString() == null
                        || stringOrRef.getString().length() == 0) {
                    writeEmptyElementWithNS(propName.getNamespaceURI(),
                            propName.getLocalPart());
                ...
                if (property.getAttributes() != null) {
                    for (Entry<QName, PrimitiveValue> attr : property
                ...
                throw new RuntimeException(
                        "Internal error. Unhandled property type '"
                                + pt.getClass() + "'");
            }
        } finally {
            exportContext.pop();
    ...
    private boolean excludeByTimeSliceFilter(Property property) {
        ...
        for (Filter timeSliceFilter : timeSliceFilters) {
            ...
            try {
                ...
                LOG.warn("Unable to evaluate time slice projection filter: "
                        + e.getMessage());
            }
        }
        return false;
        ...
        if (feature instanceof GenericFeatureCollection) {
            ...
            if (feature.getId() != null) {
                if (fidAttr.getNamespaceURI() == "") {
                    writer.writeAttribute(fidAttr.getLocalPart(),
                            feature.getId());
                } else {
                    writeAttributeWithNS(fidAttr.getNamespaceURI(),
                            fidAttr.getLocalPart(), feature.getId());
                }
            }
            ...
            for (Feature member : ((FeatureCollection) feature)) {
                ...
                if (memberFid != null
                        && referenceExportStrategy.isObjectExported(memberFid)) {
            ...
            exportContext.push(featureName);

            try {

                LOG.debug("Exporting Feature {} with ID {}", featureName,
                        feature.getId());
    ...
    private List<Property> augmentBoundedBy(Feature f) {
        ...
        for (int i = 0; i < props.size(); i++) {
            ...
            if (!gmlNs.equals(name.getNamespaceURI())
                    || name.getLocalPart().equals("location")) {
                // not a GML property or gml:location -> gml:boundedBy must be
                // included right before it
    ...
    private GmlXlinkOptions getResolveParams(Property prop,
    ...
    private void exportFeatureProperty(FeaturePropertyType pt,
            Feature subFeature, GmlXlinkOptions resolveState)
            throws XMLStreamException, UnknownCRSException,
            TransformationException {
        ...
        if (subFeature == null) {
            ...
            exportFeatureProperty(pt, (FeatureReference) subFeature,
                    resolveState, propName);
        } else {
            // normal feature
            ...
            if (subFid == null) {
                ...
                if (referenceExportStrategy.isObjectExported(subFid)) {
                    exportAlreadyExportedFeaturePropertyByReference(subFeature,
                            propName);
                } else {
                    exportFeaturePropertyByValue(propName, subFeature,
                            resolveState);
                }
            }
        }
    ...
    private void exportFeatureProperty(FeaturePropertyType pt,
            FeatureReference ref, GmlXlinkOptions resolveState, QName propName)
            throws XMLStreamException, UnknownCRSException,
            TransformationException {
        ...
        if (includeNextLevelInOutput) {
            if (pt.getAllowedRepresentation() == REMOTE) {
                ...
                if (referenceExportStrategy.isObjectExported(ref.getId())) {
                    exportAlreadyExportedFeaturePropertyByReference(ref,
                            propName);
                } else {
    ...
    private void exportAlreadyExportedFeaturePropertyByReference(Feature ref,
            QName propName) throws XMLStreamException {
        writeEmptyElementWithNS(propName.getNamespaceURI(),
                propName.getLocalPart());
    ...
    private boolean includeNextLevelInOutput(GmlXlinkOptions resolveState) {
        ...
        return maxInlineLevels == -1
                || (maxInlineLevels > 0 && currentLevel < maxInlineLevels);
    }

    private void exportFeaturePropertyByReference(QName propName,
            FeatureReference ref, boolean forceInclusionInDocument,
            GmlXlinkOptions resolveState) throws XMLStreamException {
    ...
    private void exportFeaturePropertyByValue(QName propName,
            Feature subFeature, GmlXlinkOptions resolveState)
            throws XMLStreamException, UnknownCRSException,
            TransformationException {
    ...
    private void exportGenericXmlElement(ElementNode xmlContent,
            GmlXlinkOptions resolveState) throws XMLStreamException,
            UnknownCRSException, TransformationException {
        ...
        if (elDecl != null && schemaInfoset != null) {
            ObjectPropertyType gmlPropertyDecl = schemaInfoset
                    .getGMLPropertyDecl(elDecl, elName, 0, 1, null);
            if (gmlPropertyDecl instanceof FeaturePropertyType) {
                ...
                if (children.size() == 1 && children.get(0) instanceof Feature) {
                    ...
                    exportFeatureProperty(
                            (FeaturePropertyType) gmlPropertyDecl,
                            (Feature) children.get(0), resolveState);
                    return;
                }
            }
        }
        ...
        if (xmlContent.getAttributes() != null) {
            for (Entry<QName, PrimitiveValue> attr : xmlContent.getAttributes()
                    .entrySet()) {
                writeAttributeWithNS(attr.getKey().getNamespaceURI(), attr
                        .getKey().getLocalPart(), attr.getValue().getAsText());
            }
        }
    ...
    /* Public to allow hooking */
    public boolean isPropertyRequested(QName propName) {
        if (requestedPropertyNames.isEmpty()
                && requestedPropertyPaths.isEmpty()) {
            return true;
        }

        return requestedPropertyNames.containsKey(propName)
                || isPropertyPathRequested(exportContext);
    }

    public boolean isPropertyPathRequested(Stack<QName> contextPath) {
        
        // will match partial contextPaths missing 'root' feature
        if (getRequestedPropertyPaths().isEmpty()) {
            LOG.debug("EMPTY PATHS will *NOT* PASS AT THIS POINT");
            return false;
        }
        if( contextPath.isEmpty() ) {
            LOG.debug("EMPTY CONTEXT will PASS");
            return true;
        }

        //QName[] context = (QName[])contextPath.toArray(new QName[contextPath.size()]);
        int contextLast = contextPath.size()-1;
        if (contextLast < 1) {
            LOG.debug("MATCH ROOT CONTEXT "+contextPath.get(0));
            return true;
        }

        ArrayList<ValueReference> requestedPaths = getRequestedPropertyPaths() ;
        
        for( ValueReference path : requestedPaths ) {
      
            NamespaceBindings nsContext = path.getNsContext();
            DefaultRelativeLocationPath xpath = (DefaultRelativeLocationPath) path
                .getAsXPath();
            

            List<DefaultNameStep> steps = xpath.getSteps();
            int stepsLast = steps.size()-1;

            LOG.debug("---> REQUESTED ["+stepsLast+"] "+xpath.getText() );
            LOG.debug("---> vs CONTEXT ["+contextLast+"] "+contextPath);

            
            if( stepsLast <0 ) {
                LOG.debug("EMPTY XPATH - IGNORE");
                continue;   
            }
            ...
            if( match < 0  ) {
                LOG.debug("-----===> MISMATCH "+match);
            } else if( match > stepsLast ) {                        
                LOG.debug("---------> ASSUMING PARTIAL MATCH  "+match+" vs "+stepsLast+" "+contextLast);
                return true;
            } else {
                LOG.debug("---------> ASSUMING  MATCH "+match+" vs "+stepsLast+" "+contextLast);
                return true;
            }
        }
        return false;
    }
    public Map<QName, PropertyName> getRequestedPropertyNames() {
        return requestedPropertyNames;
    }

    public Stack<QName> getExportContext() {
        return exportContext;
    }

    public ArrayList<ValueReference> getRequestedPropertyPaths() {
        return requestedPropertyPaths;
    }
